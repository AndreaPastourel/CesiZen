...
import {
  apiCreateTypeRessource,
  apiGetTypeRessourceById,
  apiUpdateTypeRessource,
} from "../../../../../services/typesRessourcesApi";
...
export default function TypeRessourceForm({
  ...
  useEffect(() => {
    if (!isEditMode || !typeRessourceId) {
      return;
    }

    const currentTypeRessourceId = typeRessourceId;
    ...
    async function loadTypeRessource() {
      ...
      try {
        const response =
          await apiGetTypeRessourceById(currentTypeRessourceId);

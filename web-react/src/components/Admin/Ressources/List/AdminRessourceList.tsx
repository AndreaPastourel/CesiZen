...
import { apiGetAllRessources, apiUpdateRessourceActive } from "../../../../services/ressourcesApi";
...
export default function AdminRessourcesList() {
  ...
  async function handleToggleActive(ressource: Ressource) {
    const newValue = !ressource.est_actif;

    setMessage(null);

    try {
      const response = await apiUpdateRessourceActive(ressource.id, newValue);
  ...
  if (isCheckingAccess) {
    return <p className={styles.loadingPage}>Vérification des droits...</p>;
  }
